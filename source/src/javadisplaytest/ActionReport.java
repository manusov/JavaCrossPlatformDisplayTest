...
void createDialogRT
      ( JFrame parentWin ,
        ...
        AbstractTableModel atm2 ,
        AbstractTableModel atm3 ,
        String longName , String vendorVersion ,
        String logName , AbstractTableModel atm4 )
    {
    ...
    while( inDialogue )
        ...
        if( select == JFileChooser.APPROVE_OPTION )
            ...
            if ( atm3 != null ) { s5 = tableReport( atm3 ); }
            // log support
            if ( logName != null ) { s6 = logName; }
            if ( atm4 != null )    { s7 = tableReport( atm4 ); }
            // save report
...
private static String tableReport ( AbstractTableModel atm ) 
    ...
    for ( int j=0; j<n; j++ )      // this cycle for rows , n = rows count
        ...
        for ( int i=0; i<m; i++ )  // this cycle for columns , m = columns count
            {
            s = " " + ( (String)atm.getValueAt( j, i ) ).trim();     // CHANGED
            report.append( s );
